...
from ansible_infra.galaxy import GalaxyClient
...
async def test_get_collection_version_info(http_redirect, ssl_certificate):
    async with CaseControlledTestServer(ssl=ssl_certificate.server_context()) as server:
        ...
        gc = GalaxyClient('https://galaxy.ansible.com', aio_session=http_redirect.session)
        task = asyncio.ensure_future(gc.get_collection_version_info('community.general'))
